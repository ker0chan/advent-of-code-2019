...
impl IntcodeInstruction
{
    ...
    fn new(params: &[u32], index: &mut usize) -> Self
    {
        match params[0] {
            1 => {
                *index += 4;
                IntcodeInstruction::Add {lhs: params[1], rhs: params[2], dest: params[3]}
            },
            2 => {
                *index += 4;
                IntcodeInstruction::Mul {lhs: params[1], rhs: params[2], dest: params[3]}
            }
...
fn execute(mut intcode: &mut [u32])
{
    ...
    while index < intcode.len() //Don't run further than expected/stop on Halt
    {
...
fn main() {
    // PART 1
    // INPUT
    let mut intcode = [1,0,0,3,1,1,2,3,1,3,4,3,1,5,0,3,2,1,10,19,1,6,19,23,2,23,6,27,2,6,27,31,2,13,31,35,1,10,35,39,2,39,13,43,1,43,13,47,1,6,47,51,1,10,51,55,2,55,6,59,1,5,59,63,2,9,63,67,1,6,67,71,2,9,71,75,1,6,75,79,2,79,13,83,1,83,10,87,1,13,87,91,1,91,10,95,2,9,95,99,1,5,99,103,2,10,103,107,1,107,2,111,1,111,5,0,99,2,14,0,0];
...
mod tests {
    ...
    fn part1_ex1() {
        ...
        assert_eq!(intcode.into_iter().map(|i:&u32| i.to_string()).collect::<Vec<String> >().join(","), "2,0,0,0,99");
    }
    ...
    fn part1_ex2() {
        ...
        crate::execute(&mut intcode);
        assert_eq!(intcode.into_iter().map(|i:&u32| i.to_string()).collect::<Vec<String> >().join(","), "2,3,0,6,99");
    }
    ...
    fn part1_ex3() {
        ...
        crate::execute(&mut intcode);
        assert_eq!(intcode.into_iter().map(|i:&u32| i.to_string()).collect::<Vec<String> >().join(","), "2,4,4,5,99,9801");
    }
    ...
    fn part1_ex4() {
        ...
        crate::execute(&mut intcode);
        assert_eq!(intcode.into_iter().map(|i:&u32| i.to_string()).collect::<Vec<String> >().join(","), "30,1,1,4,2,5,6,0,99");
    }
}